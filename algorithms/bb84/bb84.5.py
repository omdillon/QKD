...
class EnhancedBB84Protocol:
    ...
    def __init__(self,
            ...
            target_qber: float = 0.0, # unused, removed as classical noise now at reciever
            noise_type: str = 'none',
        ...
        # error rates independent of AER models
    ...
    def applyDarkCounts(self, bit: int) -> int:
        ...
        Randomly randomizes the result (simulating a false click event).
        """
        if np.random.rand() < self.DARK_COUNT_PROB:
            # Dark count acts as a random background event, effectively randomizing the result
    ...
    def bobMeasurement(self, incoming_stream: List[QuantumCircuit]):
        """
            ...
            Bob receives qubits and measures them using randomly chosen bases
            Includes classical error injection (detector noise, jitter, dark counts)
        """
        if self.verbose:
            print(f"\n{'='*85}")
    ...
    def postProcessing(self) -> Tuple[float, int, int]:
        ...
        # NOTE: injectArtificialErrors() has been removed as requested.
