...
class EnhancedBB84Protocol:
    ...
    - Distance-dependent photon loss modeling (fiber attenuation)
...
class BB84Visualizer:
    ...
    def plot_basis_matching_distribution(n_simulations: int = 100, n_qubits: int = 100, distance_km: float = 25):
        ...
        # Account for photon loss in expected value
    ...
    def plot_distance_vs_key_rate(distances_km: List[float] = None, n_qubits: int = 1000, n_trials: int = 10):
        ...
        plt.legend(fontsize=11)
        plt.tight_layout()
        plt.savefig('bb84_distance_vs_keyrate.png', dpi=300, bbox_inches='tight')
        print("[SUCCESS] Saved: bb84_distance_vs_keyrate.png")
        plt.show()
...
def interactive_simulation():
    ...
    # Parameter 4: Target QBER
    print("\n" + "-"*console_width)
    print("Target Artificial QBER:")
    print("  0.0 = No artificial errors (only Eve + noise + loss)")
    ...
    # Parameter 6: Noise strength (if applicable)
    noise_strength = 0.0
    if noise_type != 'none':
        print("\n" + "-"*console_width)
    ...
    while True:
        ...
        print("  6. Secure Key Rate vs Distance (NEW)")
