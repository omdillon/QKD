% multivariable_surface_plots_v5.m
% Experiments 5 & 6 (v.5): BB84 and B92 surface plots, three plot types per protocol.
% Three plots per protocol (6 total):
%   1. QBER surface             - raw
%   2. Mutual Information       - raw  (two surfaces: I(A;B) and I(A;E))
%   3. Secure Key Rate surface  - raw
%
% v.5 CSVs generated from:
...
% =========================================================================
% BB84  (Experiment 5 v.5)
% Grid: 51 noise steps (0%-30%) x 57 eve steps (0%-60%)
% =========================================================================
noise_vals_bb84 = linspace(0, 30, 51);
eve_vals_bb84   = linspace(0, 60, 57);
[Eve_bb84, Noise_bb84] = meshgrid(eve_vals_bb84, noise_vals_bb84);   % 51x57

if ~isfile(bb84_csv)
    warning('BB84 v.5 CSV not found: %s\nRun exp5_bb84_surface_v5.yaml first.', bb84_csv);
else
    ...
    Z_bb84_iae  = reshape(T_bb84.iae_mean,  57, 51)' * 100;
    Z_bb84_skr  = reshape(T_bb84.skr_mean,  57, 51)' * 100;
    ...
    title(ax, {'BB84 Protocol - Depolarising Channel - Mutual Information Surfaces', ...
               '(500 qubits, 40 trials per point)'}, ...
...
% =========================================================================
% B92  (Experiment 6 v.5)
% Grid: 51 noise steps (0%-15%) x 57 eve steps (0%-30%)
% =========================================================================
noise_vals_b92 = linspace(0, 15, 51);
eve_vals_b92   = linspace(0, 30, 57);
[Eve_b92, Noise_b92] = meshgrid(eve_vals_b92, noise_vals_b92);   % 51x57

if ~isfile(b92_csv)
    warning('B92 v.5 CSV not found: %s\nRun exp6_b92_surface_v5.yaml first.', b92_csv);
else
