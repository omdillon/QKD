...
% Two variants per protocol:
%   Standard - gradient colormap + translucent threshold plane, raw data
%   Gaussian - same as Standard but with Gaussian kernel smoothing applied
%
...
if ~isfile(bb84_csv)
    ...
    % Gaussian smoothing with edge replication
    [kx, ky] = meshgrid(-2:2, -2:2);
    gkernel  = exp(-(kx.^2 + ky.^2) / 2);
    gkernel  = gkernel / sum(gkernel(:));
    Z_pad = [repmat(Z_bb84(:,1), 1, 2), Z_bb84, repmat(Z_bb84(:,end), 1, 2)];
    Z_pad = [repmat(Z_pad(1,:), 2, 1); Z_pad; repmat(Z_pad(end,:), 2, 1)];
    Z_bb84_smooth = min(conv2(Z_pad, gkernel, 'valid'), ZLIM_BB84(2));
    ...
    % BB84 Gaussian (gradient + plane, smoothed)
    fig_bb84_g = figure('Position', [100 100 1200 750], 'Color', 'white', 'Visible', 'off');
    ax = axes(fig_bb84_g);
    s_data = surf(ax, Eve_bb84, Noise_bb84, Z_bb84_smooth, 'EdgeColor', 'none');
    colormap(ax, cmap_bb84);
    cb = colorbar(ax);
    cb.Label.String = 'QBER (%)'; cb.Label.FontName = FONT; cb.Label.FontSize = SZ_LABEL;
    zlim(ax, ZLIM_BB84); clim(ax, ZLIM_BB84);
    cb.Ticks = linspace(ZLIM_BB84(1), ZLIM_BB84(2), 5);
    hold(ax, 'on');
    s_plane = surf(ax, Eve_bb84, Noise_bb84, THRESH_BB84 * ones(size(Z_bb84_smooth)), ...
        'FaceAlpha', 0.25, 'FaceColor', C_PLANE_BB84, 'EdgeColor', 'none');
    xlabel(ax, 'Eve Interception Rate', 'FontName', FONT, 'FontSize', SZ_LABEL, 'FontWeight', 'bold');
    ylabel(ax, 'Noise Strength',        'FontName', FONT, 'FontSize', SZ_LABEL, 'FontWeight', 'bold');
    zlabel(ax, 'QBER (%)',              'FontName', FONT, 'FontSize', SZ_LABEL, 'FontWeight', 'bold');
    ...
    drawnow;
    ax.Position = AX_POS; cb.Position = CB_POS;
    out_g = fullfile(fileparts(bb84_csv), 'exp5_bb84_surface_qber_v3_gaussian.png');
    exportgraphics(fig_bb84_g, out_g, 'Resolution', 300);
    add_white_border(out_g, BORDER_PX);
    close(fig_bb84_g);
    fprintf('Saved: %s\n', out_g);
end
...
if ~isfile(b92_csv)
    ...
    gkernel  = exp(-(kx.^2 + ky.^2) / 2);
    gkernel  = gkernel / sum(gkernel(:));
    Z_pad = [repmat(Z_b92(:,1), 1, 2), Z_b92, repmat(Z_b92(:,end), 1, 2)];
    Z_pad = [repmat(Z_pad(1,:), 2, 1); Z_pad; repmat(Z_pad(end,:), 2, 1)];
    Z_b92_smooth = min(conv2(Z_pad, gkernel, 'valid'), ZLIM_B92(2));
    ...
    % B92 Gaussian (gradient + plane, smoothed)
    fig_b92_g = figure('Position', [100 100 1200 750], 'Color', 'white', 'Visible', 'off');
    ax = axes(fig_b92_g);
    s_data = surf(ax, Eve_b92, Noise_b92, Z_b92_smooth, 'EdgeColor', 'none');
    colormap(ax, cmap_b92);
    cb = colorbar(ax);
    cb.Label.String = 'QBER (%)'; cb.Label.FontName = FONT; cb.Label.FontSize = SZ_LABEL;
    zlim(ax, ZLIM_B92); clim(ax, ZLIM_B92);
    cb.Ticks = linspace(ZLIM_B92(1), ZLIM_B92(2), 5);
    hold(ax, 'on');
    s_plane = surf(ax, Eve_b92, Noise_b92, THRESH_B92 * ones(size(Z_b92_smooth)), ...
        'FaceAlpha', 0.25, 'FaceColor', C_PLANE_B92, 'EdgeColor', 'none');
    xlabel(ax, 'Eve Interception Rate', 'FontName', FONT, 'FontSize', SZ_LABEL, 'FontWeight', 'bold');
    ylabel(ax, 'Noise Strength',        'FontName', FONT, 'FontSize', SZ_LABEL, 'FontWeight', 'bold');
    zlabel(ax, 'QBER (%)',              'FontName', FONT, 'FontSize', SZ_LABEL, 'FontWeight', 'bold');
    ...
    drawnow;
    ax.Position = AX_POS; cb.Position = CB_POS;
    out_g = fullfile(fileparts(b92_csv), 'exp6_b92_surface_qber_v3_gaussian.png');
    exportgraphics(fig_b92_g, out_g, 'Resolution', 300);
    add_white_border(out_g, BORDER_PX);
    close(fig_b92_g);
    fprintf('Saved: %s\n', out_g);
end
