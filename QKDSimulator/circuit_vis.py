...
def _add_overlay(ax, x_bar1, x_bar2):
    ...
    # ── Channel shaded box (spans full height between barriers) ──
    ax.add_patch(Rectangle(
        (x_bar1, y_bot + 0.05), x_bar2 - x_bar1, y_top - y_bot - 0.1,
        facecolor=CHANNEL_BG, alpha=0.9, zorder=0, clip_on=False,
    ))

    # ── Wavy line — centred exactly between the two qubit wires ──
    ...
    # ── Channel caption (below the shaded box) ──
...
def main():
    # ── Separate 1-bit classical registers ──
    q  = QuantumRegister(2, 'q')
    c0 = ClassicalRegister(1, 'c_0')
    c1 = ClassicalRegister(1, 'c_1')

    # Parameterless gates with display label — avoids Qiskit rendering
    # both the label and the parameter name on separate lines in the box.
    ...
    # ── Locate barriers and add overlay ──
